...
from tensorflow.keras.layers import Input, Conv2D, Layer, BatchNormalization, PReLU, AveragePooling2D, Concatenate, Flatten, Dense, Dropout, Lambda, Subtract
...
def build_model():
    ...
    x = QConv2D(4, (3, 3), padding='same', kernel_quantizer=quantized_bits(8, 1, 1, alpha=1), bias_quantizer=quantized_bits(8, 1, 1, alpha=1), name="conv_1")(x)
    x = BatchNormalization(name="norm_1")(x)
    x = QActivation('quantized_relu(4)', name="relu_1")(x)
    ...
    x = QActivation('quantized_relu(4)', name="relu_2")(x)
    x = Dropout(0.05)(x)
...
def main():
    ...
    save_path = f'metrics/modelReduced_epochs{num_epochs}_batch{batch_size}'
    os.makedirs(save_path, exist_ok=True)

    # cregions, genPhi, genEta, cregionsMetrics = load_data(file_path)
    cregionsl, genPhi, genEta, cregions = load_data(file_path)
    # labels = calculate_labels(cregionsMetrics, genPhi, genEta)
