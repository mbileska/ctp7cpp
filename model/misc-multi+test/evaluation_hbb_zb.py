...
def evaluate_hbb_predictions(hbb_cregions, hbb_labels, model, save_path):
    ...
    plt.scatter(hbb_labels[:, 3], hbb_labels[:, 2], color='green', label='True Position 2')
    plt.scatter(hbb_predictions[:, 3], hbb_predictions[:, 2], color='purple', label='Predicted Position 2', marker='x')
    plt.xlabel('Eta')
    plt.ylabel('Phi')
...
def calculate_metrics(predictions, labels):
    ...
    accuracy_with_tolerance = np.mean((np.abs(predictions[:, 0] - labels[:, 0]) <= 3) & 
                                      ...
                                      (np.abs(predictions[:, 2] - labels[:, 2]) <= 3) &
                                      (np.abs(predictions[:, 3] - labels[:, 3]) <= 3))
