...
L1TCaloSummary<INPUT, OUTPUT>::L1TCaloSummary(const edm::ParameterSet& iConfig)
  ...
  std::string fullPathToModel("/afs/cern.ch/work/p/pdas/Run3Ntuplizer/winter24/mlclustering/CMSSW_14_1_0_pre5/src/L1Trigger/L1TCaloLayer1/data/model_new/");
...
void L1TCaloSummary<INPUT, OUTPUT>::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  float predictedPhi = outputMapped(0);  
  float predictedEta = outputMapped(1); 

  std::array<double, 2> lowerBound = {{7.50142614, 5.49990799}};
  std::array<double, 2> upperBound = {{6.50142614, 4.49990799}};
