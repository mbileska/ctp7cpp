...
#pragma HLS ARRAY_RESHAPE variable=et_calo_ad complete dim=0
        ap_ufixed<16, 8,AP_RND,AP_SAT,AP_SAT> layer12_out[N_LAYER_10];
...
#pragma HLS UNROLL
        ...
        tmp_link_out[0].range(31, 28) = layer12_out[0].range(15, 12);
        tmp_link_out[0].range(63, 60) = layer12_out[0].range(11, 8);
        tmp_link_out[0].range(95, 92) = layer12_out[0].range(7, 4);
        tmp_link_out[0].range(127, 124) = layer12_out[0].range(3, 0);
