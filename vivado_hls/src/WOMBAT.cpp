...
void WOMBAT(
    ...
    if (!loaded_weights) {
        ...
        nnet::load_weights_from_txt<norm_2_scale_t, 4>(s4, "s4.txt");
        nnet::load_weights_from_txt<norm_2_bias_t, 4>(b4, "b4.txt");
        nnet::load_weights_from_txt<weight8_t, 1008>(w8, "w8.txt");
        nnet::load_weights_from_txt<bias8_t, 4>(b8, "b8.txt");
        nnet::load_weights_from_txt<weight11_t, 16>(w11, "w11.txt");
    ...
    nnet::conv_2d_cl<input_t, layer2_t, config2>(inputs, layer2_out, w2, b2); // conv_2

    layer4_t layer4_out[OUT_HEIGHT_2*OUT_WIDTH_2*N_FILT_2];
    #pragma HLS ARRAY_PARTITION variable=layer4_out complete dim=0
    nnet::normalize<layer2_t, layer4_t, config4>(layer2_out, layer4_out, s4, b4); // norm_2

    layer5_t layer5_out[OUT_HEIGHT_2*OUT_WIDTH_2*N_FILT_2];
    #pragma HLS ARRAY_PARTITION variable=layer5_out complete dim=0
    nnet::relu<layer4_t, layer5_t, relu_config5>(layer4_out, layer5_out); // relu_2

    layer6_t layer6_out[OUT_HEIGHT_6*OUT_WIDTH_6*N_FILT_6];
    #pragma HLS ARRAY_PARTITION variable=layer6_out complete dim=0
    nnet::pooling2d_cl<layer5_t, layer6_t, config6>(layer5_out, layer6_out); // pool_2
