...
void WOMBAT(
    input_t inputs[N_INPUT_1_1*N_INPUT_2_1*N_INPUT_3_1],
    ...
    //#pragma HLS INTERFACE ap_vld port=inputs,layer16_out 
    //#pragma HLS DATAFLOW 
    #pragma HLS INLINE
    ...
    nnet::conv_2d_cl<layer6_t, layer7_t, config7>(layer6_out, layer7_out, w7, b7); // conv_2

    layer9_t layer9_out[OUT_HEIGHT_7*OUT_WIDTH_7*N_FILT_7];
    #pragma HLS ARRAY_PARTITION variable=layer9_out complete dim=0
    nnet::normalize<layer7_t, layer9_t, config9>(layer7_out, layer9_out, s9, b9); // norm_2

    layer10_t layer10_out[OUT_HEIGHT_7*OUT_WIDTH_7*N_FILT_7];
    #pragma HLS ARRAY_PARTITION variable=layer10_out complete dim=0
    nnet::relu<layer9_t, layer10_t, relu_config10>(layer9_out, layer10_out); // relu_2

    layer11_t layer11_out[OUT_HEIGHT_11*OUT_WIDTH_11*N_FILT_11];
    #pragma HLS ARRAY_PARTITION variable=layer11_out complete dim=0
    nnet::pooling2d_cl<layer10_t, layer11_t, config11>(layer10_out, layer11_out); // pool_2

    auto& layer12_out = layer11_out;
    layer13_t layer13_out[N_LAYER_13];
    #pragma HLS ARRAY_PARTITION variable=layer13_out complete dim=0
    nnet::dense<layer11_t, layer13_t, config13>(layer12_out, layer13_out, w13, b13); // dense_1

    layer15_t layer15_out[N_LAYER_13];
    #pragma HLS ARRAY_PARTITION variable=layer15_out complete dim=0
    nnet::relu<layer13_t, layer15_t, relu_config15>(layer13_out, layer15_out); // relu_3

    nnet::dense<layer15_t, result_t, config16>(layer15_out, layer16_out, w16, b16); // dense_out
